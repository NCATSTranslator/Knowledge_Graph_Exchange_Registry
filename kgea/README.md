**January 20, 2021: INCOMPLETE DRAFT WORK-IN-PROGRESS at this time... your mileage may vary...**
...
After a brief overview of the Archive design (more extensive details provided in the [KGE Archive Roadmap document](./KGE_ARCHIVE_ROADMAP.md)), this document mainly focuses on the practical activities for local development and production system deployment. 
...
1. client user authentication and authorization. 
2. a client interface (web form and, possibly, a command line interface modality) to upload KGX format-compliant **KGE File Sets** capturing the content of Biolink Model-compliant Translator knowledge graphs, with partial or complete metadata;
3. a web server application manager of the **KGE File Sets**;
4. persistence of **KGE File Sets** (and their metadata) in online network storage (i.e. an NCATS-hosted Amazon S3 bucket);
5. if complete content metadata was not already generated by the user who uploaded a given **KGE File Set**, a generation of missing content metadata for the KGX files by (re-)processing those files again through KGX;
6. publication Translator SmartAPI Registry ("Registry") entries pointing to (meta-)data access details for each **KGE File Set**, one per distinct knowledge graph.
7. client access to **KGE File Set** metadata from the Register (Note that the details and implementation of the indexing and accessing of KGE entries in the Registry will be within the technical scope of the Registry team, not this Archive code base).
8. a file streaming gateway/protocol to download **KGE File Sets**, using information from KGEA API entries retrieved from the Registry.
