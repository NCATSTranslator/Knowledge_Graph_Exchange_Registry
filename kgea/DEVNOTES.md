...
The `kgea/server/__init__.py` module is likely also overwritten by code generation but we have made some small revisions for AIOHTTP session management here, thus we have a master template copy `__init__-template.py` which can be used to restore the full code (Note: perhaps carefully do diff the template against the code generated version just to be sure...)